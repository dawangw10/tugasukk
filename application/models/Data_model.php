...
class Data_model extends CI_Model
{
    ...
    {
        $query = "SELECT `tbl_siswa`.*, `tbl_spp`.`TAHUN`, `tbl_spp`.`NOMINAL`, `tbl_kelas`.`nama_kelas`, `tbl_jurusan`.`jurusan`
        FROM `tbl_siswa`,`tbl_spp`,`tbl_kelas`,`tbl_jurusan` WHERE `tbl_siswa`.`id_kelas` = `tbl_kelas`.`id_kelas` AND `tbl_siswa`.`id_spp` = `tbl_spp`.`id_spp` AND `tbl_kelas`.`id_jurusan` = `tbl_jurusan`.`id_jurusan` AND `tbl_siswa`.`nisn` = $keyword
        ";
    ...
    public function transaksi($keyword)
    {
        ...
        FROM `tbl_pembayaran`, `tbl_siswa`,`tbl_spp`,`tbl_kelas`,`tbl_jurusan` WHERE `tbl_pembayaran`.`NISN` = `tbl_siswa`.`NISN` AND `tbl_siswa`.`id_kelas` = `tbl_kelas`.`id_kelas` AND `tbl_siswa`.`id_spp` = `tbl_spp`.`id_spp` AND `tbl_kelas`.`id_jurusan` = `tbl_jurusan`.`id_jurusan` AND `tbl_siswa`.`nisn` = $keyword
        ";
    ...
    {
        $query = $this->db->get_where('tbl_petugas');
        return $query->result();
    }
    ...
    public function tagihan($keyword){
        $this->db->where([
            'NISN' => $keyword,
            'KET' => ''
        ]);
    ...
    public function total_tagihan($keyword){
        $query = "SELECT SUM(JUMLAH_BAYAR) AS total FROM tbl_pembayaran WHERE NISN = $keyword AND KET = ''";
